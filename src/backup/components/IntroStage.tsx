...
export default function IntroStage({ onComplete, onFirstTouch, partnerName }: IntroStageProps) {
  ...
  return (
    ...
    >
      <motion.h1 
        className="font-script text-6xl text-[#8E5A47] mb-2"
      ...
      >
        <AnimatePresence mode="wait">
          <motion.p 
            ...
            className="text-[#8E5A47]/80 italic text-lg"
      ...
      >
        <Heart 
          size={64} 
          className="text-[#C16D4D]" 
          fill={progress > 0 ? "#C16D4D" : "transparent"} 
          strokeWidth={1.5}
        />
      </motion.button>

      <div className="w-full max-w-[280px] h-4 bg-[#EEDABB]/60 rounded-full overflow-hidden relative shadow-inner">
        <motion.div 
          className="h-full bg-gradient-to-r from-[#C16D4D] to-[#E3A387] rounded-full"
          initial={{ width: 0 }}
          animate={{ width: `${progress}%` }}
          transition={{ type: "spring", bounce: 0.2, duration: 0.5 }}
        />
      </div>
      <p className="mt-4 text-xs font-serif text-[#8E5A47]/60 tracking-widest">
        {Math.round(progress)}%
      </p>
    </div>
  );
